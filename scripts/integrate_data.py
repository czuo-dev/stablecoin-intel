...
def integrate_all_data(date_str: str = None):
    ...
    twitter_count = len(by_source['twitter'])
    news_count = len(by_source['news'])
    print(f"   Twitter: {twitter_count} 条 ({twitter_count/total*100:.1f}%)")
    print(f"   News: {news_count} 条 ({news_count/total*100:.1f}%)")
