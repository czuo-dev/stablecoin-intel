...
def generate_mock_tweets(count=20):
    ...
    templates = [
        "BREAKING: Circle announces USDC expansion in {region}. Major regulatory approval received. #stablecoin #USDC",
        "Tether releases Q4 attestation report. USDT reserves confirmed by BDO. Total assets: $95B #USDT",
        "Singapore MAS approves new stablecoin framework. Big win for crypto adoption in Asia! 🚀",
        "PayPal's PYUSD reaches $1B market cap. Impressive growth in just 6 months.",
    ...
    articles_templates = [
        {
            ...
            'description': 'Circle Internet Financial has obtained a Major Payment Institution license from the Monetary Authority of Singapore.',
            'category': 'company',
            'region': 'Asia',
            'coins': ['USDC']
        },
        ...
        },
        {
            ...
            'description': 'ECB publishes report highlighting potential financial stability risks from widespread stablecoin adoption.',
            'category': 'policy',
            'region': 'EU',
            'coins': []
        },
        ...
        {
            'title': 'Hong Kong Proposes HKD Stablecoin Sandbox',
            'description': 'HKMA invites licensed institutions to participate in Hong Kong dollar stablecoin pilot program.',
            'category': 'policy',
            'region': 'Asia',
            'coins': []
        },
        {
            'title': 'Paxos Launches USDP on Multiple Blockchains',
            'description': 'Regulated stablecoin issuer Paxos expands Pax Dollar to Solana and Polygon networks.',
            'category': 'company',
            'region': 'US',
            'coins': ['USDP']
        },
        {
            ...
            'description': 'Total stablecoin market capitalization reaches new all-time high driven by institutional adoption.',
            'category': 'market',
            'region': 'Global',
            'coins': ['USDC', 'USDT']
        },
        {
            ...
            'description': 'Brazilian central bank grants approval to local fintech for BRL-backed stablecoin.',
            'category': 'policy',
            'region': 'LATAM',
            'coins': []
        },
        {
            'title': 'IMF Calls for International Stablecoin Standards',
            'description': 'International Monetary Fund proposes global framework to regulate cross-border stablecoins.',
            'category': 'policy',
            'region': 'Global',
            'coins': []
        },
...
def save_mock_data(date_str=None):
    ...
    print(f"✅ 已生成Twitter数据: {twitter_file} ({len(tweets)} 条)")
    ...
    print(f"✅ 已生成新闻数据: {news_file} ({len(news)} 条)")
...
if __name__ == '__main__':
    ...
    print(f"\n✅ 模拟数据生成完成!")
    print(f"\n现在可以运行: python scripts/integrate_data.py")